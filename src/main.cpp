...
CImg<unsigned char> resizeKeepAspectRatio(CImg<unsigned char> srcImage,
  ...
  try {
    /*
     * The size of the final image is determined by the scaling factor, and
  ...
  return srcImage.resize((int)(srcImage.width() * scale),
                         (int)(srcImage.height() * scale), -100, -100, 6);
}

int ValidImage(std::uint8_t *ImageBytes) {
  const static std::vector<std::uint8_t> GIFBytesOne = {0x47, 0x49, 0x46,
                                                        0x38, 0x37, 0x61};
  const static std::vector<std::uint8_t> GIFBytesTwo = {0x47, 0x49, 0x46,
                                                        0x38, 0x39, 0x61};
  const static std::vector<std::uint8_t> PNGBytes = {0x89, 0x50, 0x4E, 0x47,
                                                     0x0D, 0x0A, 0x1A, 0x0A};
  const static std::vector<std::uint8_t> BMPBytes = {0x42, 0x4D};
  const static std::vector<std::uint8_t> JPGBytes = {0xFF, 0xD8, 0xFF};
  const static std::vector<std::uint8_t> JPEGBytes = {0x00, 0x00, 0x00, 0x0C,
                                                      0x6A, 0x50, 0x20, 0x20};
  ...
  const static std::vector<std::uint8_t> CompressedTGA = {
      0x0, 0x0, 0xA, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
  const static std::vector<std::uint8_t> DeCompressedTGA = {
      0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};

  const static std::array<std::vector<std::uint8_t>, 13> All = {
      GIFBytesOne, GIFBytesTwo,    PNGBytes,       BMPBytes, JPGBytes,
      JPEGBytes,   TIFFMonoChrome, TIFFOne,        TIFFTwo,  TIFFThree,
      TIFFFour,    CompressedTGA,  DeCompressedTGA};
...
string readOrderNumberFromFile(std::filesystem::path filePath) {
  try {
    // Check if a text document with name "overlay.txt" exists and use the
    // containing number as order number
...
int high_low(int high, int low) { return (high - low) / 2; }
...
CImg<unsigned char> getTextbox(const char *text, int initialSize,
   ...
   */
  while (((coveredArea < (toBeCovered - tolerance)) ||
          (coveredArea > (toBeCovered + tolerance)))) {
    try {
...
void resizeAndStore(fs::path file) {
  ...
  fs::path resizedFilePath =
      file.parent_path() /
      (fs::path(file.stem().string() + resizedAppendix).string() +
...
void printVersion() { std::cout << "Version: " << VERSION << std::endl; }
...
int main(int argc, char *argv[]) {
  ...
  textbox =
      getTextbox(orderNumber,
                 sqrt(newimage_width * newimage_height / 100 * coveragePercent),
                 static_cast<float>(newimage_width * newimage_height),
                 coveragePercent, tolerance);

  auto start = std::chrono::steady_clock::now();
  std::vector<std::future<void>> futures;

  // Reserve 16K of memory that can be deleted just in case we run out of
  // memory
  ...
  try {
    ...
    // Delete the reserved memory so we can print an error message before
    // exiting
  ...
  auto diff = end - start;
  std::cout << "Converted " << files.size() << " images in "
