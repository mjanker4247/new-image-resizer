...
private:
	...
	// length of text at end of progress bar
...
public:
    ProgressBar(unsigned int total, unsigned int width, char complete, char incomplete) :
            total_ticks {total}, bar_width {width - length}, complete_char {complete}, incomplete_char {incomplete} {}

    ProgressBar(unsigned int total, unsigned int width) : total_ticks {total}, bar_width {width - length} {}
    ...
    void display() const
    {
        ...
        std::cout << "] " << int(progress * 100.0) << "% "
                  << int(float(time_elapsed) / 1000.0) << "s\r";
        std::cout.flush();
    }
